...
SUBROUTINE InflowWind_CalcOutput( Time, InputData, p, &
   ...
   ! zhu:2022年2月18日修改，原因是降低输出频率提高计算效率    write(*,*)'InflowWind_CalcOutput::flag::1'       !zhu
   ...
   ELSEIF ( SIZE(InputData%PositionXYZ,DIM=2) /= SIZE(OutputData%VelocityUVW,DIM=2) ) THEN
      CALL SetErrStat( ErrID_Fatal," Programming error: Position and Velocity arrays are not sized the same.",  &
            ErrStat, ErrMsg, RoutineName)
   ENDIF
   ! zhu:2022年2月18日修改，原因是降低输出频率提高计算效率    write(*,*)'InflowWind_CalcOutput::flag::2' 
   !-----------------------------
   ...
   CALL CalculateOutput( Time, InputData, p, &
      ...
      CALL SetErrStat( TmpErrStat, TmpErrMsg, ErrStat, ErrMsg, RoutineName )
   ! zhu:2022年2月18日修改，原因是降低输出频率提高计算效率    write(*,*)'InflowWind_CalcOutput::flag::3' 
   !-----------------------------
   ...
   ! zhu:2022年2月18日修改，原因是降低输出频率提高计算效率    write(*,*)'InflowWind_CalcOutput::flag::4'    
   !-----------------------------
