...
! =========  Type_Zhu  =======
  TYPE, PUBLIC :: Type_Zhu
    REAL(ReKi)  :: InputTheta(2)    !<Rotor angle [rad]
    REAL(ReKi)  :: PitchAngle    !<Blade Pitch angle [rad]
    REAL(ReKi)  :: RotorMoment   !<Moment get from controller
    REAL(ReKi)  :: RotorSpeed    !<RotorSpeed by calculate
    REAL(ReKi)  :: avrSWAP(108)
    REAL(ReKi)  :: pitch_In
    REAL(ReKi)  :: InTime        !For discom time
    REAL(ReKi)  :: GenSpeed      !generator speed
    REAL(ReKi)  :: HorWindV      !hub wind speed
    REAL(ReKi)  :: MoBeGenToSh       !Moment between Generator and Shift
    INTEGER(4)  :: GenFlag       !the flag tell the program that the generator has started
    REAL(ReKi),DIMENSION(:), ALLOCATABLE  :: output
    REAL(ReKi)  :: towerRootMotion(3) !set the towerMotion
    REAL(ReKi)  :: towerRootAngle(3)  !set the towerRootAnlge
    REAL(ReKi)  :: towerRootVel(3)    !set the tower velocity
    REAL(ReKi)  :: towerRootAngleVel(3) !set the tower angle velocity
    REAL(ReKi),DIMENSION(:,:),ALLOCATABLE  :: towerNodePosition !save the tower origion node position
    REAL(ReKi),DIMENSION(:,:),ALLOCATABLE  :: NodePosition
    INTEGER(4)  :: NumNode
  END TYPE Type_Zhu
